...
class DefaultController extends Controller
{
    public function indexAction()
    {
        ...
        $statement = $connection->prepare("SELECT * FROM soutanance WHERE `Date_Soutenance`>=(CURDATE()-INTERVAL 1 MONTH)");
        $statement->execute();
        ...
        $statement->execute();
        $LastMonth = $statement->fetchAll();
        $statement = $connection->prepare("SELECT * FROM `tuto_user` WHERE `roles`='a:1:{i:0;s:10:\"ROLE_ADMIN\";}'");
        $statement->execute();
        $admis = $statement->fetchAll();
        $statement = $connection->prepare("SELECT * FROM `tuto_user` WHERE `roles`='a:1:{i:0;s:9:\"ROLE_PROF\";}'");
        $statement->execute();
        $prof = $statement->fetchAll();
        $statement = $connection->prepare("SELECT * FROM `tuto_user` WHERE `roles`='a:1:{i:0;s:9:\"ROLE_STUD\";}'");
        $statement->execute();
        $stud = $statement->fetchAll();
        $statement = $connection->prepare("SELECT count(*) as nbr,MONTH(`Date_Debut`) as month FROM `stage` WHERE affect = 0 GROUP BY MONTH(`Date_Debut`)");
        $statement->execute();
        $stage0 = $statement->fetchAll();
        $statement = $connection->prepare("SELECT count(*) as nbr,MONTH(`Date_Debut`) as month FROM `stage` WHERE affect = 1 GROUP BY MONTH(`Date_Debut`)");
        $statement->execute();
